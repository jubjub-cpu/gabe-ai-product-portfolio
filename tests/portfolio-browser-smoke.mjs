...
try {
  ...
  assert.equal(await page.locator(".project-card").count(), 11, "Eleven central project cards should render");
  ...
  const coldChainCard = page.locator(".project-card").filter({ hasText: "ColdChain Sentinel" });
  assert.equal(await coldChainCard.getByRole("link", { name: "Open live demo" }).getAttribute("href"), "https://jubjub-cpu.github.io/coldchain-sentinel/");
  assert.equal(await coldChainCard.getByRole("link", { name: "Repo" }).getAttribute("href"), "https://github.com/jubjub-cpu/coldchain-sentinel");
  const civicCaseCard = page.locator(".project-card").filter({ hasText: "CivicCase Redact" });
  assert.equal(await civicCaseCard.getByRole("link", { name: "Open live demo" }).getAttribute("href"), "https://jubjub-cpu.github.io/civiccase-redact/");
  assert.equal(await civicCaseCard.getByRole("link", { name: "Repo" }).getAttribute("href"), "https://github.com/jubjub-cpu/civiccase-redact");
  const trustPathCard = page.locator(".project-card").filter({ hasText: "TrustPath Studio" });
  assert.equal(await trustPathCard.count(), 1, "TrustPath card should render once");
  assert.equal(await trustPathCard.getByRole("link", { name: "Open live demo" }).getAttribute("href"), "https://jubjub-cpu.github.io/trustpath-studio/");
  assert.equal(await trustPathCard.getByRole("link", { name: "Repo" }).getAttribute("href"), "https://github.com/jubjub-cpu/trustpath-studio");
  ...
  assert.equal(await mobilePage.locator(".project-card").count(), 11, "Eleven project cards should render on mobile");
  ...
  console.log(JSON.stringify({ target: deployedBaseUrl ? "deployed" : "local", cards: 11, featured: 7, additional: 4, docuTraceLinks: 3, reviewFlowLinks: 3, frameForgeLinks: 3, queueCastLinks: 3, evalDeckLinks: 2, flowReplayLinks: 2, voiceGaugeLinks: 2, coldChainLinks: 2, civicCaseLinks: 2, trustPathLinks: 2, desktopOverflow: false, mobileOverflow: false, consoleErrors: 0, failedRequests: 0 }));
} finally {
